/****************************************************************************************/
/******************* [System] 3D TI (Strong, BiSe, kx-ky)             *******************/
...
double const    Kplane  = 0.1 * M_PI;       // perpendicular momentum to the 2D subsystems
...
main(int argc, char **argv)
{
    ...
    FILE *f1;
    f1 = fopen("BiSe_Zak_Ex_16_kz_01","wt");
        ...
        for (p = 0; p <= Ncut-1; p++)
        {
            for (q = 0; q <= Ncut-1; q++)
            {
                ...
                for (it = 0; it <= Tmax-1; it++)
                {
                    ...
                    Ek1 = C + 0.5 * (2.0 - cos(Otime) - cos(Kperp)) + 2.0 * D2 * (1.0 - cos(Kplane));
                    
                    d1 = A1 * sin(Otime);
                    d2 = A1 * sin(Kperp);
                    d3 = M + 2.0 * B1 * (2.0 - cos(Otime) - cos(Kperp)) + 2.0 * B2 * (1.0 - cos(Kplane));
                    d4 = A2 * sin(Kplane);
                    d5 = 0;
                    ...
                    FHuuRe += 1.0/(2.0*M_PI*Tmax) * cos((p-q)*Otime) * (Ek1 - dnorm - Bloch * Alpha * d2 * Dd1);
                    FHuuIm += 1.0/(2.0*M_PI*Tmax) * sin((p-q)*Otime) * (Ek1 - dnorm - Bloch * Alpha * d2 * Dd1);
                    
                    FHddRe += 1.0/(2.0*M_PI*Tmax) * cos((p-q)*Otime) * (Ek1 - dnorm + Bloch * Alpha * d2 * Dd1);
                    FHddIm += 1.0/(2.0*M_PI*Tmax) * sin((p-q)*Otime) * (Ek1 - dnorm + Bloch * Alpha * d2 * Dd1);

                    BetaRe = (- pow(d1,2) + pow(d2,2)) / (2.0*dnorm*(dnorm - d3)*(pow(d1,2) + pow(d2,2)));
                    BetaIm = - 2.0*d1*d2 / (2.0*dnorm*(dnorm - d3)*(pow(d1,2) + pow(d2,2)));
                    
                    FHudRe += 1.0/(2.0*M_PI*Tmax) * Bloch * (BetaIm * cos((p-q)*Otime) + BetaRe * sin((p-q)*Otime)) * d4 * Dd1;
                    FHudIm += 1.0/(2.0*M_PI*Tmax) * Bloch * (- BetaRe * cos((p-q)*Otime) + BetaIm * sin((p-q)*Otime)) * d4 * Dd1;
                    
                    FHduRe += 1.0/(2.0*M_PI*Tmax) * Bloch * (BetaIm * cos((p-q)*Otime) - BetaRe * sin((p-q)*Otime)) * d4 * Dd1;
                    FHduIm += 1.0/(2.0*M_PI*Tmax) * Bloch * (BetaRe * cos((p-q)*Otime) + BetaIm * sin((p-q)*Otime)) * d4 * Dd1;
// at TRIM
//                    BetaRe = - 1.0 / (2.0*dnorm*(dnorm - d3));
//                    BetaIm = 0;
//
//                    FHudRe += 1.0/(2.0*M_PI*Tmax) * Bloch * BetaRe * sin((p-q)*Otime) * d4 * Dd1;
//                    FHudIm += 1.0/(2.0*M_PI*Tmax) * Bloch * (- BetaRe) * cos((p-q)*Otime) * d4 * Dd1;
//
//                    FHduRe += 1.0/(2.0*M_PI*Tmax) * Bloch * (- BetaRe) * sin((p-q)*Otime) * d4 * Dd1;
//                    FHduIm += 1.0/(2.0*M_PI*Tmax) * Bloch * BetaRe * cos((p-q)*Otime) * d4 * Dd1;
//
                }
