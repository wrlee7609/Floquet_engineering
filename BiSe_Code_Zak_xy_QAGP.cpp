...
double const    Kplane  = 0.600000 * M_PI;      // perpendicular momentum to the 2D subsystems
...
double FHuuRe (double x, void * params) {
    ...
    double Ek1 = C + 0.500000 * (2.000000 - cos(x) - cos(Kperp)) + 2.000000 * D2 * (1.000000 - cos(Kplane));
    double d1 = A1 * sin(x);
    double d2 = A1 * sin(Kperp);
    double d3 = M + 2.000000 * B1 * (2.000000 - cos(x) - cos(Kperp)) + 2.000000 * B2 * (1.000000 - cos(Kplane));
    double d4 = A2 * sin(Kplane);
    double d5 = 0;
    double Dd1 = A1 * cos(x);
    ...
    double FHuuRe = cos((p-q)*x) * (Ek1 - dnorm - Alpha * Bloch * d2 * Dd1) / (2.000000*M_PI);
    return FHuuRe;
}
...
double FHuuIm (double x, void * params) {
    ...
    double FHuuIm = sin((p-q)*x) * (Ek1 - dnorm - Alpha * Bloch * d2 * Dd1) / (2.000000*M_PI);
    return FHuuIm;
}
...
double FHddRe (double x, void * params) {
    ...
    double FHddRe = cos((p-q)*x) * (Ek1 - dnorm + Alpha * Bloch * d2 * Dd1) / (2.000000*M_PI);
    return FHddRe;
}
...
double FHddIm (double x, void * params) {
    ...
    double FHddIm = sin((p-q)*x) * (Ek1 - dnorm + Alpha * Bloch * d2 * Dd1) / (2.000000*M_PI);
    return FHddIm;
}
...
double FHudRe (double x, void * params) {
    ...
    double BetaRe = (- pow(d1,2) + pow(d2,2)) / (2.0*dnorm*(dnorm - d3)*(pow(d1,2) + pow(d2,2)));
    double BetaIm = - 2.0*d1*d2 / (2.0*dnorm*(dnorm - d3)*(pow(d1,2) + pow(d2,2)));
    
    double FHudRe = (cos((p-q)*x) * BetaIm + sin((p-q)*x) * BetaRe) * Bloch * d4 * Dd1 / (2.000000*M_PI);
    return FHudRe;
}
...
double FHudIm (double x, void * params) {
    ...
    double FHudIm = (- cos((p-q)*x) * BetaRe + sin((p-q)*x) * BetaIm) * Bloch * d4 * Dd1 / (2.000000*M_PI);
    return FHudIm;
}
...
double FHduRe (double x, void * params) {
    ...
    double FHduRe = (cos((p-q)*x) * BetaIm - sin((p-q)*x) * BetaRe) * Bloch * d4 * Dd1 / (2.000000*M_PI);
    return FHduRe;
}
...
double FHduIm (double x, void * params) {
    ...
    double FHduIm = (cos((p-q)*x) * BetaRe + sin((p-q)*x) * BetaIm) * Bloch * d4 * Dd1 / (2.000000*M_PI);
    return FHduIm;
}
...
main(int argc, char **argv)
{
    ...
    FILE *f1;
    f1 = fopen("BiSe_Zak_Ex_16_kz_06","wt");
