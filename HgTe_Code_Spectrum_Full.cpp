...
double const	A		= 0.6;				// off-diagonal dispersion (in unit 2|D|)
double const	B		= -0.6;				// spin-orbit coupling strength (in unit 2|D|)
double const	M		= -0.3;				// mass gap (in unit 2|D|) (+ trivial; - nontrivial)
...
{	
	...
	FILE *f1;
	f1 = fopen("HgTeDosA6B6Mm3K0Low","wt");
	...
	for (i = 3; i <= 5; i += 2)		// Equil 45, Low Field (3,21), High Field (23,181)
	{
	...
	for (iw = Fmax*rank/Ncpu; iw <= Fmax*(rank+1)/Ncpu-1; iw++)
	{
		for (n = 0; n <= Ncut-1; n++)
		{
			...
			if (Mod == 1) {
				...
				for (ik = 0; ik <= Kcut-1; ik++)
				{
					if ( fabs((ik-(Kcut-1.0)/2.0)*Komega) <= M_PI ) {
						...
						LocDosSub[iw+Fmax*(n-rank)/Ncpu] += 
							Komega/(2.0*M_PI) * 1.0/M_PI * 0.5*Gamma / (pow(ReG1,2) + pow(0.5*Gamma,2));
					}
				}
			}
			
			
			// Mod 2 : Side - Finite Field
			...
			if (Mod == 2) {
				...
				// 2-band retarded local Green's function (orbital H1)
				
				gsl_complex kGreenB = gsl_matrix_complex_get (kGreen2B, n, n);
				
				// Semi-local Dos (orbital H1)
				
				LocDosSub[iw+Fmax*(n-rank)/Ncpu] = - GSL_IMAG(kGreenB) / M_PI;
