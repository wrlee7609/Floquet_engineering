...
int const       MCC         = 10000/Ncpu;       /* number of function calls for Monte Carlo integration (2000000/Ncpu, 10000/Ncpu) */
...
/* INPUT: parameters for Monte Carlo mothod */
...
/***************************************************/
/* SUBLOUTINE: density of state (DOS)              */
/***************************************************/
...
double f_kDOS (double *vars, size_t dim, void *params)
{
    ...
    kDOS = - vars[0] / pow(2.0*M_PI,2) * (GSL_IMAG (z1) + GSL_IMAG (z2)) / M_PI / Ncpu;
...
/******************************************************/
/* SUBLOUTINE: Local DOS                              */
/******************************************************/

double Subroutine_LDOS (int rank, int n, double Field, double omega)
{
    /* DECLARATION: variables and arrays */
    ...
    struct gsl_params params_n = { rank, n, Field, omega, Coeff_Fit[0], Coeff_Fit[1], Coeff_Fit[2], Coeff_Fit[3], Coeff_Fit[4] };
...
double f_Qn (double *vars, size_t dim, void *params)
{
    /* DECLARATION: variables for function */
    
    /* vars[0]: variable for frequency domain 1 */
    /* vars[1]: variable for frequency domain 2 */
    ...
    int p, q, Ncut;
    double LDOS_Sub, LDOS_1, LDOS_2, DistDiff, Qn_Int;
    
    /* PARALLELIZING: MPI */
    
    Qn_Int = 0;
    
    if (vars[1] != vars[0] + n * Freq) {
        
        for (p = - Ncut; p <= Ncut; p++)
        {
            for (q = - Ncut; q <= Ncut; q++)
            {
                LDOS_Sub = Subroutine_LDOS (rank, p, Field, vars[0]);
                
                MPI_Reduce(&LDOS_Sub, &LDOS_1, 1, MPI_DOUBLE, MPI_SUM, rank, MPI_COMM_WORLD);   // NOTICE: root = rank
                
                LDOS_Sub = Subroutine_LDOS (rank, q, Field, vars[1]);
                
                MPI_Reduce(&LDOS_Sub, &LDOS_2, 1, MPI_DOUBLE, MPI_SUM, rank, MPI_COMM_WORLD);   // NOTICE: root = rank
                
                DistDiff = 1.0 / (exp((vars[0] + p * Freq) / Temp) + 1.0) - 1.0 / (exp((vars[1] + q * Freq) / Temp) + 1.0);
                
                Qn_Int += LDOS_1 * LDOS_2 * DistDiff / (vars[1] - vars[0] - n * Freq) / Ncpu;
            }
        }
...
/******************************************************/
/* SUBLOUTINE: Weight function Qn                     */
/******************************************************/

double Subroutine_Qn (int rank, int n, double Field, int Ncut)
{
    /* DECLARATION: variables and arrays */
    
    double xl[2] = {-Wcut, -Wcut};
    double xu[2] = {Wcut, Wcut};
    double Qn_Sub, error;
    
    /* INTEGRATION: Monte Carlo method (MISER) */
    
    struct gsl_params params_n = { rank, n, Field, 0, 0, 0, 0, 0, 0 };
    gsl_monte_function F_Qn = { &f_Qn, 2, &params_n };
    ...
    {
        gsl_monte_miser_state *s = gsl_monte_miser_alloc (2);
        gsl_monte_miser_integrate (&F_Qn, xl, xu, 2, calls, r, s, &Qn_Sub, &error);
        gsl_monte_miser_free (s);
    }
...
main(int argc, char **argv)
{
    ...
    int n, p, q, Ncut, iField, iomega;
    double Field, omega;
    double Qn_Sub, LDOS_Sub, LDOS;
    double TunCond_Wn, TunCond_Qn, TunCond_Q0;
...
//            if ((n == 4 && Field >= 0.93) || n > 4) { }
            ...
            if (n == 0) {
                ...
                TunCond_Wn = 0.5 * gsl_sf_bessel_Jn(0,2.0*Field/pow(Freq,2)) * (gsl_sf_bessel_Jn(1,2.0*Field/pow(Freq,2)) - gsl_sf_bessel_Jn(-1,2.0*Field/pow(Freq,2))) / Field;
            ...
            Qn_Sub = Subroutine_Qn (rank, n, Field, Ncut);
            
            MPI_Reduce(&Qn_Sub, &TunCond_Qn, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        ...
        if (Field == 0.5) {
            ...
            for (n = -N0cut; n <= N0cut; n++) {
                ...
                for (iomega = 0; iomega <= Fmax-1; iomega++)
                {
                    ...
                    LDOS_Sub = Subroutine_LDOS (rank, n, Field, omega);
                    
                    MPI_Reduce(&LDOS_Sub, &LDOS, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
                    
                    /* SAVE: data on LDOS */
                    
                    if (rank == 0) {
                        
                        printf("%f %f %f %e\n", Freq, Field, omega + n * Freq, LDOS);
                        fprintf(f1, "%f %f %f %e\n", Freq, Field, omega + n * Freq, LDOS);
                    }
                }
            }
