...
int const       MCC         = 2000000/Ncpu;     /* number of function calls for Monte Carlo integration */
int const       Freqcut     = 81;               /* grid number of optical frequency */
double const    dFreq       = 0.01;             /* grid size of optical frequency */
double const    Freq0       = 0.20;             /* initial value inside the window for optical frequency */
...
double const	Field       = 1.00;             /* electric field */
...
double *Subroutine_Fit (double Freq)
{
...
{
    int comp;
    double Freq, Coeff_Fit_0, Coeff_Fit_1, Coeff_Fit_2, Coeff_Fit_3, Coeff_Fit_4;
};
...
double g (double *vars, size_t dim, void *params)
{
    ...
    double Freq = ((struct gsl_params *) params) -> Freq;
    ...
    for (j = 0; j <= 1; j++)
    {
        ...
        for (k = 0; k <= 1; k++)
        {
            ...
            for (n = 1; n <= Ncut-2-harmonics; n++)
            {
                ...
                z4 = gsl_matrix_complex_get (Ga, n+(1-j)*harmonics + 1, n+j*harmonics);
                z5 = gsl_matrix_complex_get (Ga, n+(1-j)*harmonics - 1, n+j*harmonics);
                ...
                for (l = 1; l <= Ncut-2; l++)
                {
                    ...
                    z3 = gsl_complex_sub (gsl_matrix_complex_get (Gr, n+(1-j)*harmonics + 1, l), gsl_matrix_complex_get (Gr, n+(1-j)*harmonics - 1, l));
                    z4 = gsl_complex_sub (gsl_matrix_complex_get (Gr, n+(1-j)*harmonics, l+1), gsl_matrix_complex_get (Gr, n+(1-j)*harmonics, l-1));
                    z5 = gsl_complex_sub (gsl_complex_sub (gsl_matrix_complex_get (Gr, n+(1-j)*harmonics + 1, l+1), gsl_matrix_complex_get (Gr, n+(1-j)*harmonics + 1, l-1)),
                                          gsl_complex_sub (gsl_matrix_complex_get (Gr, n+(1-j)*harmonics - 1, l+1), gsl_matrix_complex_get (Gr, n+(1-j)*harmonics - 1, l-1)));
                    z6 = gsl_matrix_complex_get (Ga, l, n+j*harmonics);
                    z7 = gsl_complex_sub (gsl_matrix_complex_get (Ga, l+1, n+j*harmonics), gsl_matrix_complex_get (Ga, l-1, n+j*harmonics));
...
/***********************************************************/
/* SUBLOUTINE: dynamical conductivity vs optical frequency */
/***********************************************************/

double Subroutine_OptCond (double Freq, int comp, int rank)
{
    /* DECLARATION: variables and arrays */
    ...
    Coeff_Fit = Subroutine_Fit (Freq);
    
    /* INTEGRATION: Monte Carlo Method (MISER) */
    
    struct gsl_params params_p = { comp, Freq, Coeff_Fit[0], Coeff_Fit[1], Coeff_Fit[2], Coeff_Fit[3], Coeff_Fit[4] };
...
main(int argc, char **argv)
{
    ...
    FILE *f1;
    f1 = fopen("OptCond_Ec10_T002_E100_020to100","wt");
    ...
    for (iFreq = 0; iFreq <= Freqcut-1; iFreq++)
    {
        ...
        for (comp = 0; comp <= 1; comp++)
        {
            ...
            OptCond_Sub = Subroutine_OptCond (Freq, comp, rank);
