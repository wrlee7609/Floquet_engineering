...
int const       MCC         = 2000000;          /* number of function calls for Monte Carlo integration */
int	const		Ncpu        = 10;				/* total number of CPUs 200 */
int const       Freqcut     = Ncpu;             /* grid number of optical frequency */
double const    dFreq       = 0.01;             /* grid size of optical frequency */
double const    Freq0       = 0.2;              /* initial value inside the window for optical frequency */
...
double const    Ec          = 10.0;             /* Dirac band cutoff (2.0, 6.0, 10.0) */
double const    Field       = 0.0001;           /* electric field (0.0001, 0.1, 0.2) */
...
/*********************************************************/
/* SUBLOUTINE: optical conductivity vs optical frequency */
/*********************************************************/
...
double *Subroutine_OptCond (int rank)
{
    ...
    for (iFreq = rank*Freqcut/Ncpu; iFreq <= (rank+1)*Freqcut/Ncpu-1; iFreq++)
    {
        ...
        for (comp = 2; comp <= 2; comp++)   // temporarily focus on Re[Sigma_yx]
        {
            /* INTEGRATION: Monte Carlo Method (PLAIN / MISER / VEGAS) */
            
            struct gsl_params params_p = { comp, iFreq, Coeff_Fit[0], Coeff_Fit[1], Coeff_Fit[2], Coeff_Fit[3], Coeff_Fit[4] };
...
main(int argc, char **argv)
{
    ...
    FILE *f1;
    f1 = fopen("OptCond_Ec10_T002_E00001_2to3","wt");
